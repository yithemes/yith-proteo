...
if ( ! function_exists( 'yith_proteo_scripts' ) ):
	...
	function yith_proteo_scripts() {
		...
		wp_enqueue_style( 'yith-proteo-linearicons', get_template_directory_uri() . '/third-party/icon-font.min.css', array(), '1.0.0' );
		wp_enqueue_style( 'yith-proteo-simple-line-icons', get_template_directory_uri() . '/third-party/simple-line-icons.css', array(), '2.4.1' );
		...
		$args = [ 'jquery' ];
		function_exists( 'WC' ) && $args[] = 'selectWoo';

		wp_enqueue_script( 'yith-proteo-themejs', get_template_directory_uri() . '/js/theme.js', $args, YITH_PROTEO_VERSION, true );
		wp_localize_script( 'yith-proteo-themejs', 'yith_proteo', [
			'stickyHeader' => apply_filters( 'yith_proteo_enable_sticky_header', get_theme_mod( 'yith_proteo_header_sticky', 'yes' ) )
		] );
		...
		if ( get_theme_mod( 'yith_proteo_google_font' ) !== '' ) {
			wp_enqueue_style( 'yith-proteo-google-font', get_theme_mod( 'yith_proteo_google_font', 'https://fonts.googleapis.com/css?family=Montserrat:300,400,500,600,700&display=swap' ), array( 'yith-proteo-style' ) );
		}
...
add_action( 'admin_enqueue_scripts', 'yith_proteo_admin_scripts', 15 );